...
class Model1(SmartPixModel):
    ...
    ################################################################################################################################################
    def makeQuantizedModelHyperParameterTuning5(self):
        def model_builder(hp):
            # ── B) Architecture hyperparams ──────────────────────────────────────────
            # separately tune rows and cols
            ...
            q_input1 = QActivation(activation=quantized_bits(14, 0), name="q_input_1")(input1)
            q_input2 = QActivation(activation=quantized_bits(14, 0), name="q_input_2")(input2)
            q_input3 = QActivation(activation=quantized_bits(14, 0), name="q_input_3")(input3)
            q_input4 = QActivation(activation=quantized_bits(14, 0), name="q_input_4")(input4)
            ...
            bias_quantizer=quantized_bits(12, 0, alpha=1),
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_dir = f"{self.modelName.lower()}_quantized_hp5q_12w0i_i14_results_{timestamp}"
        os.makedirs(save_dir, exist_ok=True)
        print(f"\n✓ Trial artifacts will be saved in: {save_dir}/\n")
        ...
        tuner = SaveModelRandomSearch(
            ...
            max_trials=120,
            executions_per_trial=2,
            project_name="hp_search_5rows_12w0i_i14_quantized_matching",
            directory="./hyperparameter_tuning_5q_12w0i_i14",   # keep KT logs in one place
            overwrite=True,                        # avoid weird resume behavior
            save_dir=save_dir,
            objective_name="val_binary_accuracy",
        )
    ...
    def makeQuantizedModelHyperParameterTuning4(self):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_dir = f"{self.modelName.lower()}_quantized_hp4q_12w0i_i14_results_{timestamp}"
        os.makedirs(save_dir, exist_ok=True)
        print(f"\n✓ Trial artifacts will be saved in: {save_dir}/\n")
        ...
        tuner = SaveModelRandomSearch(
            ...
            max_trials=120,
            executions_per_trial=2,
            project_name="hp_search_4rows_12w0i_i14_quantized_matching",
            directory="./hyperparameter_tuning_4q_12w0i_i14",   # keep KT logs in one place
            overwrite=True,                        # avoid weird resume behavior
            save_dir=save_dir,
            objective_name="val_binary_accuracy",
        )
    ...
    def makeQuantizedModelHyperParameterTuning3(self):
        def model_builder(hp):
            # ── B) Architecture hyperparams ──────────────────────────────────────────
            # separately tune rows and cols
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_dir = f"{self.modelName.lower()}_quantized_hp3q_12w0i_i14_results_{timestamp}"
        os.makedirs(save_dir, exist_ok=True)
        print(f"\n✓ Trial artifacts will be saved in: {save_dir}/\n")
        ...
        tuner = SaveModelRandomSearch(
            ...
            max_trials=120,
            executions_per_trial=2,
            project_name="hp_search_3rows_12w0i_i14_quantized_matching",
            directory="./hyperparameter_tuning_3q_12w0i_i14",   # keep KT logs in one place
            overwrite=True,                        # avoid weird resume behavior
            save_dir=save_dir,
            objective_name="val_binary_accuracy",
        )
    ...
    def makeQuantizedModelHyperParameterTuning2(self):
        def model_builder(hp):
            # ── B) Architecture hyperparams ──────────────────────────────────────────
            # separately tune rows and cols
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_dir = f"{self.modelName.lower()}_quantized_hp2q_12w0i_i14_results_{timestamp}"
        os.makedirs(save_dir, exist_ok=True)
        print(f"\n✓ Trial artifacts will be saved in: {save_dir}/\n")
        ...
        tuner = SaveModelRandomSearch(
            ...
            max_trials=120,
            executions_per_trial=2,
            project_name="hp_search_2rows_12w0i_i14_quantized_matching",
            directory="./hyperparameter_tuning_2q_12w0i_i14",   # keep KT logs in one place
            overwrite=True,                        # avoid weird resume behavior
            save_dir=save_dir,
            objective_name="val_binary_accuracy",
        )
