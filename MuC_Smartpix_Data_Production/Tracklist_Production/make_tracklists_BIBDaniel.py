...
#Takes in the list of file paths to .slcio's, and the total number of tracks
#Returns a "tracks" which is a list a track events, where each track looks like
#track = [cota, cotb, p, ops.flp, ylocal, zglobal, pt, t, hit_pdg]
def slcioToTracks(file_list,track_total):
    tracks=[]
    track_count=0
    break_loop=False
    count = 0
    ...
    for file_path in file_list:
        ...
        for ievt,event in enumerate(reader):
            ...
            for hit in vtxBarrelHits: 
                if track_count>track_total-1:
                    print(f"\nReached total number of tracks ({track_total}). Breaking loop.\n")
                    break_loop=True
                    break
...
def writeTracklists(tracks, numFiles, binsize, fileStart = 0):
    ...
    for fileNum in range(numFiles):
        ...
        with open(output_file.replace("*", str(fileNum + fileStart)), 'w') as file:
...
#     print(f"tracks1 length is {len(tracks1)} which we think is too long for the RAM to handle")
#     print(f"First save numFilesToSaveEarly*binsize={tracksToSaveEarly} tracks ({numFilesToSaveEarly} tracklists) so can clear them from memory")
#     if len(tracks1) < tracksToSaveEarly:
#         raise RuntimeError("I am confused how this would be reached since numFilesToSaveEarly*binsize>200000 seems incorrect")
#     writeTracklists(tracks1[:tracksToSaveEarly],numFilesToSaveEarly,binsize)
#     newTracks1 = tracks1[tracksToSaveEarly:]
#     snapshot1 = tracemalloc.take_snapshot()
#     # tracks1 = tracks1[tracksToSaveEarly:]
#     # del tracks1[:tracksToSaveEarly]
#     del tracks1
#     gc.collect()
#     snapshot2 = tracemalloc.take_snapshot()
#     stats = snapshot2.compare_to(snapshot1, 'lineno')
#     print(f"Memory released by deleting large list: {stats[0].size_diff / 10**6:.2f} MB")
#     print(f"Now new tracks1 length is {len(newTracks1)} so hopefully that saved some RAM")
#     fileStart2 = tracksToSaveEarly#qq
#     print("now starting tracks2")
#     tracks2 = slcioToTracks(file_list2,ops.track_total-len(newTracks1))
#     tracks = newTracks1 + tracks2
# elif len(tracks1) == ops.track_total:
#     print("tracks2 wasn't necessary")
#     filestart2 = 0
#     tracks = tracks1
# else:
#     print("Somehow got more than ops.track_total number of tracks???")
#     raise RuntimeError("Something unexpected happened with generating tracklists...look over code")
